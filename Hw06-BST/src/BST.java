...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		if (subtree.isEmpty()) {
			...
			TreeIterator min = (TreeIterator) findMin();
			BSTNode minInTree = min.currentNode;
			// add subtree.root in findMax!
			TreeIterator max = (TreeIterator) findMax(subtree.root);
			BSTNode maxInSubtree = max.currentNode;

			if (maxInSubtree.data < minInTree.data) {
				minInTree.left = subtree.root;
				subtree.root.parent = minInTree;
				// add size
				size += subtree.size;
			}
			return;
